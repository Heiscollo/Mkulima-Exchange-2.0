...
export function Negotiate() {
  ...
  const handleAcceptCounter = async () => {
    ...
    try {
      ...
      notifySuccess('Counter accepted', 'Order updated successfully.');
      navigate('/orders');
    } catch (error: any) {
      notifyError('Could not accept', error?.response?.data?.message || 'Unable to accept counter.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleRejectCounter = async () => {
    ...
    try {
      ...
      notifySuccess('Counter rejected', 'The order has been updated.');
      navigate('/orders');
    } catch (error: any) {
      notifyError('Could not reject', error?.response?.data?.message || 'Unable to reject counter.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="rounded-[32px] border border-[#F4ECE1] bg-white p-6 shadow-sm md:p-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto]">
          ...
          <button onClick={handleSendCounter} disabled={saving} className="self-end rounded-2xl bg-gradient-kenya px-5 py-4 font-black text-white disabled:cursor-not-allowed disabled:opacity-70">
            <MessageSquare size={16} className="mr-2 inline" /> Send counter
          </button>
        </div>
        <div className="mt-6 flex flex-wrap gap-3">
          <button onClick={handleAcceptCounter} disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-[#008D41]/10 px-5 py-3 font-black text-[#008D41] disabled:opacity-70"><CheckCircle2 size={16} /> Accept counter</button>
          <button onClick={handleRejectCounter} disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-[#E32636]/10 px-5 py-3 font-black text-[#E32636] disabled:opacity-70"><X size={16} /> Reject counter</button>
        </div>
      </div>
    </div>
  );
